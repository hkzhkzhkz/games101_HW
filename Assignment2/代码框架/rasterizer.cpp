...
static bool insideTriangle(int x, int y, const Vector3f* _v)
{
	...
	Vector3f _dot(x + 0.5, y + 0.5, 0);
	Vector3f vec0 = _v[2] - _v[0];
	Vector3f vec1 = _v[0] - _v[1];
	Vector3f vec2 = _v[1] - _v[2];
	int z0 = (vec0.cross(_dot - _v[0])).z();
	int z1 = (vec1.cross(_dot - _v[1])).z();
	int z2 = (vec2.cross(_dot - _v[2])).z();
...
void rst::rasterizer::draw(pos_buf_id pos_buffer, ind_buf_id ind_buffer, col_buf_id col_buffer, Primitive type)
{
    ...
    for (auto& i : ind) /// process one triangle each loop
    {
        Triangle t;
        Eigen::Vector4f v[] = { /// mvp transformation
