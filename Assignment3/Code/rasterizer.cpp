...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
    ...
    for (const auto& t:TriangleList)
    {
        ...
        ///三角形未进行projection的坐标(已进行mv将摄像机移到原点)
        ...
        Eigen::Matrix4f inv_trans = (view * model).inverse().transpose();//求逆再转置
        Eigen::Vector4f n[] = {///顶点法线的计算
        ...
        // Also pass view space vertice position //已经设置每个顶点的坐标|法线|颜色
...
void rst::rasterizer::set_pixel(const Vector2i &point, const Eigen::Vector3f &color)
{
	...
	int ind = point.y() * width + (width - point.x());
    frame_buf[ind] = color;
}
